...
function wrapperPlugin(plugin_info) {
	...
	*/
			...
			if (!seenCells[cellStr]) {
				...
				if (isCellOnScreen(bounds, cell)) {
					...
					if (cellData) {
                        ...
                        if(missingStops > 1 && !ignoredCellsMissingGyms[cellStr]) {
							cellLayerGroup.addLayer(fillCell(cell, settings.colors.cellsMissingGyms.color, settings.colors.cellsMissingGyms.opacity * 1/(20/missingStops) ));
                        } else if (missingStops < 1 && !ignoredCellsExtraGyms[cellStr]) {
						...
						if (zoom > 15) {
                            ...
                            cellData.notClassified.forEach(coverLevel17Cell);
						}
    ...
    // Compute howmany possible stops are in the cell making it redundant to select gyms/pokestops
    function computeUniqueNotClassified(cellData) {
        let duplicateNotClassified = 0;
        let dup = {};
        Object.keys(cellData.notClassified).forEach(t => {dup[t] = false; });

        //foreach celldata.notClassified x foreach celldata.notclassified y if (x != y && x.s2cell == y.s2cell) DuplicateNotClassified++;
        Object.keys(cellData.notClassified).forEach(x =>
        {
            Object.keys(cellData.notClassified).forEach(y =>
            {
                if(x != y &&
                   cellAreEqual(cellData.notClassified[x], cellData.notClassified[y]) &&
                   dup[x] == false)
                {
                   duplicateNotClassified++;
                   dup[y] = true;

                }

            });
            Object.keys(cellData.stops).forEach(y =>
            {
                if(cellAreEqual(cellData.notClassified[x], cellData.stops[y]))
               {
                   duplicateNotClassified++;
               }
            });
            Object.keys(cellData.gyms).forEach(y =>
            {
               if(cellAreEqual(cellData.notClassified[x], cellData.gyms[y]))
               {
                   duplicateNotClassified++;
               }
            });
        });
        //console.log(duplicateNotClassified);
        return cellData.notClassified.length - duplicateNotClassified;
    }

    function cellAreEqual(celldata1, celldata2)
    {
        if( S2.S2Cell.FromLatLng(celldata1, poiCellLevel).getLatLng().lat == S2.S2Cell.FromLatLng(celldata2, poiCellLevel).getLatLng().lat &&
            S2.S2Cell.FromLatLng(celldata1, poiCellLevel).getLatLng().lng == S2.S2Cell.FromLatLng(celldata2, poiCellLevel).getLatLng().lng )
        {
            return true;
        }
        return false;
	...
	function computeMissingStops(cellData) {
		...
		const stops = cellData.stops.length + computeUniqueNotClassified(cellData);
	...
	function computeMissingGyms(cellData) {
		...
		const sum = totalGyms + cellData.stops.length + computeUniqueNotClassified(cellData);
		if (sum < 2)
			return 0 - totalGyms;
